!function(){
	var bP={};	
	var b=30, bb=300, height=800, buffMargin=1, minHeight=14;
	var c1=[-120, 50], c2=[-50, 250], c3=[-50, 300]; //Column positions of labels.
	var colors =["#00008B", "#DC3912",  "#FF9900","#109618", "#990099", "#0099C6","#FCDC3B","#EE6363"];
	...
	bP.partData = function(data,p){
		...
		sData.keys=[
			...
			d3.set(data.map(function(d){ return d[2];})).values().sort(function(a,b){ return ( a<b? -1 : a>b ? 1 : 0);})		
		];
...
// 	console.log(data.keys[2][i]);
// 	}
			
		mainbar.append("line").attr("class","strokes").attr("x1","-200")
			.attr("y1",function(d){return d.middle+5;}).attr("x2","-130")
			.attr("y2",function(d){ return d.middle+5;}).attr("stroke-opacity","0")
			.attr("stroke-width","5")
	...
	function transitionPart(data, id, p){
		...
		mainbar.select(".strokes").transition().duration(500)
			.attr("x1","-200")
			.attr("y1",function(d){ return d.middle+5;}).attr("x2","-130")
			.attr("y2",function(d){ return d.middle+5;})
	...
	bP.selectSegment = function(data, m, s){
		data.forEach(function(k){
			...
			//if(parseInt(newdata.keys[2][index]) > 0){
				var unselectedBar = d3.select("#"+k.id).select(".part"+(1-m)).select(".mainbars")
					.selectAll(".mainbar");
				unselectedBar.select('.strokes').style("stroke-opacity",1).style("stroke","green");	
			//}
			
			for (i in newdata.keys[2])	{
			if(parseInt(newdata.keys[2][i]) > 0){console.log("");console.log(newdata.keys[2][i])}
			
			}
			var selectedBar = d3.select("#"+k.id).select(".part"+m).select(".mainbars")
				.selectAll(".mainbar").filter(function(d,i){ return (i==s);});
			selectedBar.select(".strokes").style("stroke-opacity",1).style("stroke","green");			
	...
	bP.deSelectSegment = function(data, m, s){
		data.forEach(function(k){
			...
			var unselectedBar = d3.select("#"+k.id).select(".part"+(1-m)).select(".mainbars")
				.selectAll(".mainbar");
			unselectedBar.select('.strokes').style("stroke-opacity",0).style("stroke","green");	
			var selectedBar = d3.select("#"+k.id).select(".part"+m).select(".mainbars")
				.selectAll(".mainbar").filter(function(d,i){ return (i==s);});
			selectedBar.select(".strokes").style("stroke-opacity",0);			
